...
func TestIntegration_FloorDiscoveryToChart(t *testing.T) {
	...
	// The catalog publishes the floor the namespace declared, unchanged, and
	// leaves it empty where the namespace declared none.
	devicesByFloor := map[string][]string{}
	for _, d := range catalog.Devices {
		if want := floorDevices()[d.ID].Floor; d.Floor != want {
			t.Errorf("%s: floor = %q, want the declared %q", d.ID, d.Floor, want)
		}
		devicesByFloor[d.Floor] = append(devicesByFloor[d.Floor], d.ID)
	}
	if len(devicesByFloor[""]) != 1 {
		t.Errorf("want exactly the one device with no declared floor, got %v", devicesByFloor[""])
	}
	...
	for floor, want := range devicesByFloor {
		...
		t.Run("floor="+floor, func(t *testing.T) {
			...
			for _, ser := range resp.Series {
				...
				if declared := floorDevices()[ser.Key].Floor; declared != floor {
					t.Errorf("series %s is on floor %q, not the requested %q", ser.Key, declared, floor)
				}
				if len(ser.Values) == 0 {
...
func TestIntegration_UnknownFloorIsAJSON400(t *testing.T) {
	...
	get(t, base, token, "/series?window=today&interval=1h&floors=floor9", http.StatusBadRequest, &body)

	msg, _ := body["error"].(string)
	if !strings.Contains(msg, "floor9") || !strings.Contains(msg, "floors") {
