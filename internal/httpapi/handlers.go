...
func (s *Server) resolveDeviceFilter(w http.ResponseWriter, r *http.Request) (map[string]config.DeviceConfig, bool) {
	...
	// id starts with it, so `floors=groundfloor` saves the caller enumerating the
	...
	// device with no room id has an UNKNOWN floor, not a floor of its own, so it
	// contributes nothing here and can never be selected by floors= — see
...
type catalogEntry struct {
	...
	// Floor is derived from Room, not configured: the floorplan taxonomy spells
	// a room id "<floor>.<slug>", so the floor is already in there. It is empty
	// when unknown — a device still on the deprecated free-text location has no
	// floor to report, and greenhouse does not invent one.
...
// its room, the floor derived from that room id, and an `environment_fields`
// hint of the fields it writes.
//
